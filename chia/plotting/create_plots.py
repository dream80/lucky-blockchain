...
def get_farmer_public_key(alt_fingerprint: Optional[int] = None) -> G1Element:
    ...
    if sk_ent is None:
        raise RuntimeError("No keys, please run 'lucky keys add', 'lucky keys generate' or provide a public key with -f")
...
def get_pool_public_key(alt_fingerprint: Optional[int] = None) -> G1Element:
    ...
    if alt_fingerprint is not None:
        ...
        sk_ent = keychain.get_first_private_key()
    if sk_ent is None:
        raise RuntimeError("No keys, please run 'lucky keys add', 'lucky keys generate' or provide a public key with -p")
