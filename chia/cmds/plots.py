...
def show_plots(root_path: Path):
    ...
    print(
        "Add with 'lucky plots add -d [dir]' and remove with"
        + " 'lucky plots remove -d [dir]'"
        + " Scan and check plots with 'lucky plots check'"
    )
    print()
...
def plots_cmd(ctx: click.Context):
    ...
    if not root_path.is_dir():
        raise RuntimeError("Please initialize (or migrate) your config directory with 'lucky init'")
